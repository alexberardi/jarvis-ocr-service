...
PG_URL = "postgresql+psycopg2://jarvis:pw@postgres:5432/jarvis_ocr"
...
def test_existing_database_is_left_alone(engines):
    ...
    assert "jarvis_ocr" in engines
    assert "postgres" not in engines


def test_missing_database_is_created(engines):
    engines["_behaviour"]["jarvis_ocr"] = {
    ...
    assert admin.executed == ['CREATE DATABASE "jarvis_ocr"']
...
def test_missing_database_detected_without_pgcode(engines):
    ...
    engines["_behaviour"]["jarvis_ocr"] = {
        "connect_error": _op_error(
            None, 'FATAL:  database "jarvis_ocr" does not exist'
        )
    }

    assert ensure_database(PG_URL) is True
    assert engines["postgres"].executed == ['CREATE DATABASE "jarvis_ocr"']
...
def test_message_match_is_anchored_to_this_database(engines):
    ...
    engines["_behaviour"]["jarvis_ocr"] = {
        "connect_error": _op_error(None, 'FATAL:  database "jarvis_recipes" does not exist')
    }
...
def test_engines_are_always_disposed(engines):
    ensure_database(PG_URL)
    assert engines["jarvis_ocr"].disposed
